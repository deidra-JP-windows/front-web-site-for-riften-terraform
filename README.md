...
## ドキュメント一覧
要件整理書をたたき台として、他ドキュメントを Github Copilot で生成しています。
- 要件整理書 → 要件定義書 → その他ドキュメント の流れ
- 不足分はサンプルの値か外部ドキュメント参照を記述させ、そこから正しい値をいれ修正させる
- 要件整理書に反映させたら、他ドキュメントも要件定義書に従って修正させる
- GPT-4.1 だとある程度対話は必要
